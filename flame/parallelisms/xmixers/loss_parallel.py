...
class LossParallel(ParallelStyle):
    ...
    def _prepare_input_fn(
        ...
        weight, hidden_states, labels, bias, num_logits_to_keep, fuse_linear_and_cross_entropy = inputs
        ...
        if hidden_states.placements != desired_input_layouts:
            hidden_states = hidden_states.redistribute(placements=desired_input_layouts, async_op=True)

        if not isinstance(labels, DTensor):
            labels = DTensor.from_local(
                labels, device_mesh, [Replicate()]
            )
        
        if labels.placements != desired_input_layouts:
            labels = labels.redistribute(placements=desired_input_layouts, async_op=True)

        return weight.to_local(), hidden_states.to_local(), labels.to_local(), bias.to_local() if bias is not None else bias, num_logits_to_keep, fuse_linear_and_cross_entropy
    ...
    def _prepare_output_fn(output_layouts, use_local_output, mod, outputs, device_mesh):
        if not isinstance(output1, DTensor) and output1 is not None:
            output1 = DTensor.from_local(
                output1, device_mesh, [Replicate()]
            )
